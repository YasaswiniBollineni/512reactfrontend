    ...
    <section className='section-sm lg:section-lg bg-section'>
      <div className="container mx-auto">
        ...
        <div className='text-center mb-7 lg:mb-[70px]'>
          <h2 className='h2 mb-3 lg:mb-[10px]'>Pick a pricing plan</h2>
          <p className='text-center mb-7 lg:mb-[70px]'>pick a pricing plan and get started your journey with us for build your body and mind</p>
        </div>
        {/* cards */}
          ...
          {pricing.map((card, currentIndex) => {
            ...
            return (
              ...
              className='bg-white w-full max-w-[368px] min-h-[668px] h-full cursor-pointer relative' key={currentIndex}>
                {/* card top */}
                <div className={`${index === currentIndex ? 'bg-orange text-white' : 'bg-white text-heading'} 
                ...
                <div className='px-[30px] pt-[18px] pb-[30px]'>
                    ...
                    {list.map((item, index) => {
                      ...
                      return (
                        <li className='border border-stroke-3 p-4 rounded-md flex items-center gap-4' key={index}>
                          <div className='flex justify-center bg-green-100 w-7 h-7 text-2xl text-green-300 rounded-full'>{icon}</div>
                          {name}
                        </li>
                      );
                    })}
                  ...
                  <button className={`${index === currentIndex ? 'bg-orange' : 'border-orange'}
                   w-full h-[50px] rounded border border-orange flex items-center justify-center text-orange font-medium transition relative`}>{buttonText}
                    {/* icon */}
                    <div className={`${index === currentIndex ? 'bg-orange-200' : 'bg-orange'} 
                    w-[50px] h-[50px] flex justify-center items-center absolute right-0 rounded text-white`}>{buttonIcon}</div>
                  </button>
                  </div>
                </div>
              </div>
